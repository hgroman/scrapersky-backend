...
- User Acceptance: ✅ APPROVED / ⏸️ PENDING / ❌ REJECTED
...
```json
{
  ...
  "crms": ["brevo", "hubspot"],
  "action": "select",
  "message": "Selected 2 contacts for 2 CRM(s), 4 queued for processing"
}
```
...
**Tests Created:**
1. `test_select_contacts_for_brevo` - ✅ PASS / ❌ FAIL
2. `test_select_contacts_for_multiple_crms` - ✅ PASS / ❌ FAIL
3. `test_unselect_contacts` - ✅ PASS / ❌ FAIL
4. `test_filter_contacts_by_crm_status` - ✅ PASS / ❌ FAIL
5. `test_invalid_crm_name` - ✅ PASS / ❌ FAIL
...
**Checklist Completed:** ✅ YES / ⏸️ PARTIAL / ❌ NO
...
**Scenarios Tested:**
1. [Scenario 1] - ✅ APPROVED / ❌ REJECTED
2. [Scenario 2] - ✅ APPROVED / ❌ REJECTED

**User Feedback:**
> [Paste user's comments/feedback here]

**Issues Raised:**
- [Issue 1]: [Description] - Status: ✅ FIXED / ⏸️ DEFERRED / ❌ WONT FIX
...
**Severity:** 🔴 Critical
**Description:** Initial Phase 1 implementation only set `{crm}_sync_status` but did NOT set `{crm}_processing_status`. This violated the dual-status pattern used throughout ScraperSky (Pages, Domains, Sitemap Files, Local Businesses) and would have prevented Phase 2 schedulers from finding queued contacts.
**Workaround:** None - required immediate fix
**Fix Required:** ✅ YES
**Blocking Next Phase:** ❌ NO (fixed before Phase 2 via WO-015.7)
**Fixed In:** WO-015.7 implementation - Added dual-status adapter logic to `/crm/select` endpoint
**Fix Details:**
- Now sets `{crm}_processing_status = 'Queued'` when action = "select"
- Clears `{crm}_processing_error` when selecting
- Resets `{crm}_processing_status` to NULL when action = "unselect"
- Returns `queued_count` in response to show how many were queued for processing
- Follows exact pattern from Pages Router (WF7), Domains Router (WF4), etc.
...
2. **Database Fields Ready:** All `{crm}_sync_status` and `{crm}_processing_status` fields exist and are writable
3. **Dual-Status Adapter Implemented:** Marking as 'Selected' AUTOMATICALLY sets `{crm}_processing_status = 'Queued'` - Phase 2 schedulers will query this field
4. **Scheduler Query Pattern:** Phase 2 should query `WHERE {crm}_processing_status = 'Queued'` to find contacts ready for sync
5. **Frontend Integration:** [Describe if frontend was updated, or if that's pending]
...
- [ ] WO-015_INDEX.md (update Phase 1 status to ✅ COMPLETE)
...
- [ ] WO-015_INDEX.md updated (Phase 1 status = ✅ COMPLETE)
...
### Ready for Phase 2: ✅ YES / ❌ NO / ⏸️ BLOCKED
