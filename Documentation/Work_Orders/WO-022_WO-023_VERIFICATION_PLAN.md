# WO-022 & WO-023 Verification Plan

**Objective:** Verify that all database standardization changes (Enum renames, Foreign Keys, and Column Type fixes) are correctly implemented and compatible with the application code.

**Executor:** Partner AI (Cascade) / User
**Target:** Production/Staging Database & Local Codebase

---

## 1. Prerequisites

*   [ ] **Database:** Ensure migrations [20251120000000_fix_enums_and_fks.sql](file:///Users/henrygroman/development/python-projects/ScraperSky-Back-End-WorkSpace/scraper-sky-backend/supabase/migrations/20251120000000_fix_enums_and_fks.sql) and [20251120000001_fix_local_business_status_type.sql](file:///Users/henrygroman/development/python-projects/ScraperSky-Back-End-WorkSpace/scraper-sky-backend/supabase/migrations/20251120000001_fix_local_business_status_type.sql) have been executed.
*   [ ] **Code:** Ensure `src/models/` files are synced (LocalBusiness, Domain, Sitemap, Place).

---

## 2. Automated Verification Script

Create and run the following script `tests/verify_wo022_wo023_comprehensive.py`. This script tests:
1.  **WO-023 Fix:** Can save `LocalBusiness` with `PlaceStatusEnum` values (e.g., 'Maybe').
2.  **WO-022 Enum Rename:** Can save `Domain` with `SitemapCurationStatusEnum`.
3.  **WO-022 FKs:** Can save `SitemapFile` with valid `tenant_id`.
4.  **WO-022 FK Enforcement:** **Fails** to save record with invalid `tenant_id` (Integrity Check).
...
If the automated script passes, perform these quick manual checks to ensure system stability.

### 3.1 Check Application Startup
Run the application server to ensure no import/startup errors occur due to model definitions.
```bash
# Example command (adjust for your setup)
uvicorn src.main:app --reload --port 8000
```
*   **Success Criteria:** Server starts without `SQLAlchemyError` or `ImportError`.

### 3.2 Verify Router Endpoints (Optional but Recommended)
1.  **GET /local-businesses**: Ensure listing works (verifies `status` enum serialization).
2.  **GET /domains**: Ensure listing works (verifies `sitemap_curation_status` serialization).

---

## 4. Rollback Procedure (If Verification Fails)

If **TEST 1 (Enum)** fails:
*   Check `src/models/local_business.py`. Ensure `name="place_status_enum"`.
*   Check DB: `\d local_businesses`. Ensure column `status` is type `place_status_enum`.

If **TEST 3 (FK)** fails (i.e., invalid tenant accepted):
*   The Foreign Key constraint is missing in the database.
*   Re-run migration `20251120000000_fix_enums_and_fks.sql`.
