...
async def process_domain_with_own_session(
    ...
    try:
        ...
        if not domain_url:
            raise ValueError(f"Could not convert standardized domain to URL: {std_domain}")
        ...
        await update_job_status_orm(job_id, "processing", domain_id=domain_obj.id, metadata=metadata)
        ...
        if site_metadata is not None:
            ...
            await update_job_status_orm(job_id, "completed", result_data=result_metadata)
        else:
            # Include timing even on metadata extraction failure
            await update_job_status_orm(job_id, "failed", last_error="Failed to extract metadata from domain", result_data=result_metadata)

        logger.info(f"Successfully completed processing domain {domain_url} (job: {job_id}) in {processing_time:.2f} seconds")

    except Exception as e:
        end_time = datetime.now(timezone.utc)
        processing_time = (end_time - start_time).total_seconds()
        error_message = str(e)
        logger.error(f"Error processing domain {domain}: {error_message}", exc_info=True)
        ...
        await update_job_status_orm(job_id_str, "failed", last_error=error_message, result_data=error_metadata)
...
async def get_or_create_domain_orm(domain_url: str) -> Optional[Domain]:
    ...
    try:
        async with get_background_session() as session:
            ...
            if domain_obj:
                logger.info(f"Found existing domain record for {domain_url}, id: {domain_obj.id}")
                return domain_obj
            else:
                ...
                try:
                    ...
                    insert_stmt = pg_insert(Domain).values(
                        domain=domain_url,
                    ...
                    result_refetch = await session.execute(stmt_select) # Reuse select statement
                    domain_obj_refetched = result_refetch.scalar_one_or_none()

                    if domain_obj_refetched:
                        logger.info(f"Successfully created or found domain for {domain_url} after insert attempt, id: {domain_obj_refetched.id}")
                        return domain_obj_refetched
                    else:
                        logger.error(f"CRITICAL: Domain {domain_url} not found even after INSERT attempt.")
                        return None

                except Exception as insert_exc:
                    await session.rollback() # Rollback on insert error
                    logger.error(f"Error during domain insert for {domain_url}: {insert_exc}", exc_info=True)
                    return None # Indicate failure

    except Exception as e:
        logger.error(f"Error in get_or_create_domain_orm for {domain_url}: {e}", exc_info=True)
        return None
    ...
    try:
        ...
        async with get_background_session() as session:
            async with session.begin(): # Use transaction block for the update
                ...
                await session.flush() # Optional: flush to see changes immediately if needed
