...
def get_engine():
    ...
    logger.info(
        f"Creating database engine with: pool_size={pool_settings['pool_size']}, "
...
def get_db_url() -> str:
    ...
    if not db_url:
        ...
        if os.getenv("SUPABASE_POOLER_HOST"):
            ...
            if (
                pooler_user
                and "." in pooler_user
                and pooler_user.startswith("postgres.")
            ):
                ...
                logger.warning(
                    f"Reformatting pooler username to required format: {db_user}"
                )
    ...
    if db_url and not db_url.startswith(
        ...
        if db_url.startswith(("postgres://", "postgresql://")):
            ...
            db_url = (
                ...
                if "://" in db_url
                else f"postgresql+asyncpg://{db_url}"
            )
    ...
    for param in tenant_related_params:
        if param in query_params:
            ...
            logger.warning(
                f"Removed {param} from database URL as per architectural mandate"
            )
    ...
    logger.info(
        "Using Supavisor connection pooler as required by architectural mandate"
    )
...
async def get_db_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    try:
        ...
        logger.error(
            f"Exception during database session, rolling back: {e}", exc_info=True
        )
