...
class Contact(Base, BaseModel):
    ...
    domain_id: Column[uuid.UUID] = Column(PGUUID(as_uuid=True), ForeignKey("domains.id", ondelete="CASCADE"), nullable=False, index=True)
    page_id: Column[uuid.UUID] = Column(PGUUID(as_uuid=True), ForeignKey("pages.id", ondelete="CASCADE"), nullable=False, index=True)
    email: Column[str] = Column(Text, nullable=False, index=True)
    email_type: Column[Optional[ContactEmailTypeEnum]] = Column(
        SQLAlchemyEnum(ContactEmailTypeEnum, name="contact_email_type_enum", create_type=False),
        nullable=True
    )
    ...
    source_job_id: Column[Optional[uuid.UUID]] = Column(PGUUID(as_uuid=True), ForeignKey("jobs.job_id"), nullable=True)
    ...
    job = relationship("Job") # Consider adding back_populates="contacts" to Job model if needed
