...
async def list_sitemap_files(
    ...
    deep_scrape_curation_status: Optional[SitemapDeepCurationStatusEnum] = Query(
        None,
        description="Filter by deep scrape curation status (New, Selected, etc.)"
    ),
    ...
    sitemap_type: Optional[str] = Query(
        None,
        description="Filter by sitemap type (e.g., Standard, Index)"
    ),
    discovery_method: Optional[str] = Query(
        None,
        description="Filter by discovery method (e.g., robots_txt, common_path)"
    ),
    page: int = Query(1, ge=1, description="Page number"),
    size: int = Query(15, ge=1, le=200, description="Items per page"), # Default size 15 as per spec
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error retrieving sitemap files.")

@router.put(
    "/status",
    response_model=Dict[str, int], # Response model matches service: {"updated_count": N, "queued_count": M}
...
async def update_sitemap_files_status_batch(
    ...
    if not user_id:
        logger.error("User ID not found in JWT token for batch update status operation.")
    ...
    if not update_request.sitemap_file_ids:
        ...
        logger.warning(f"User {user_id} called batch curation update with empty ID list.")
    ...
    try:
        ...
        logger.info(f"Batch curation status update by user {user_id} completed. Results: {result_counts}")
        return result_counts
    except HTTPException as http_exc:
        # Re-raise HTTPExceptions directly
        raise http_exc
    except Exception as e:
        logger.error(f"Error during batch curation status update by user {user_id}: {e}", exc_info=True)
        # Service layer exceptions will cause rollback via session.begin()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error performing batch status update.")
...
async def create_sitemap_file(
    ...
    try:
        ...
        logger.info(f"Sitemap file created successfully by user {user_id} (pending commit).")
        return created_sitemap_file
    except ValueError as ve:
        # Handle potential UUID conversion error if user_id isn't a valid UUID string
        logger.error(f"Invalid user ID format for create operation: {user_id} - {ve}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid user identifier.")
    except Exception as e:
        logger.error(f"Error during sitemap file creation by user {user_id}: {e}", exc_info=True)
        # Re-raise a generic server error
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error creating sitemap file.")
...
async def get_sitemap_file(
    ...
    session: AsyncSession = Depends(get_db_session),
    current_user: Dict[str, Any] = Depends(get_current_user) # Added for consistency, remove if not needed
):
...
async def update_sitemap_file(
    ...
    logger.info(f"User {user_id} attempting to update sitemap file ID {sitemap_file_id}.")
    ...
    try:
        ...
        if updated_sitemap_file is None:
            logger.warning(f"Sitemap file {sitemap_file_id} not found for update by user {user_id}.")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Sitemap file not found")

        logger.info(f"Sitemap file {sitemap_file_id} updated successfully by user {user_id} (pending commit).")
        return updated_sitemap_file
    except ValueError as ve:
        # Handle potential UUID conversion error if user_id isn't a valid UUID string
        logger.error(f"Invalid user ID format for update operation: {user_id} - {ve}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid user identifier.")
    except HTTPException as http_exc:
        # Re-raise HTTPExceptions directly (like the 400 Bad Request from empty data)
        raise http_exc
    except Exception as e:
        logger.error(f"Error during sitemap file update for ID {sitemap_file_id} by user {user_id}: {e}", exc_info=True)
        # Re-raise a generic server error
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error updating sitemap file.")
...
async def delete_sitemap_file(
    ...
    logger.info(f"User {user_id or 'Unknown'} attempting to delete sitemap file ID {sitemap_file_id}.")

    try:
        deleted = await sitemap_files_service.delete(session, sitemap_file_id)
        if not deleted:
            logger.warning(f"Sitemap file {sitemap_file_id} not found for deletion by user {user_id or 'Unknown'}.")
            # Service returns False if rowcount is 0
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Sitemap file not found")

        logger.info(f"Sitemap file {sitemap_file_id} deleted successfully by user {user_id or 'Unknown'} (pending commit).")
        ...
        logger.error(f"Error during sitemap file deletion for ID {sitemap_file_id} by user {user_id or 'Unknown'}: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error deleting sitemap file.")
