...
# Create router
router = APIRouter(prefix="/api/v3/batch_page_scraper", tags=["batch_page_scraper"])
...
async def scan_domain(
    ...
    try:
        ...
        user_id = (
            str(current_user.get("id", "")) if current_user.get("id") else "system"
        )
        ...
        async with session.begin():
            ...
            result = await page_processing_service.initiate_domain_scan(
                session=session, base_url=base_url, user_id=user_id, max_pages=max_pages
            )
        ...
        logger.info(
            f"Background task added for domain: {base_url}, job_id: {result['job_id']}"
        )
        ...
        raise HTTPException(
            status_code=400,
            detail={"message": "Invalid input parameter", "error": str(e)},
        )
...
async def get_job_status(
    ...
    try:
        ...
        logger.debug(
            f"Database parameters: raw_sql={raw_sql}, no_prepare={no_prepare}, statement_cache_size={statement_cache_size}"
        )
...
async def create_batch_endpoint(
    ...
    try:
        ...
        max_concurrent = (
            request.max_concurrent if request.max_concurrent is not None else 5
        )
        ...
        async with session.begin():
            ...
            batch_result = await initiate_batch_processing(
                session=session, domains=domains, user_id=user_id, options=batch_options
            )
        ...
        # Create status URL
        status_url = (
            f"/api/v3/batch_page_scraper/batch/{batch_result['batch_id']}/status"
        )
    ...
    try:
        ...
        async with session.begin():
            ...
            batch_status = await get_batch_progress(
                session=cast(Session, session), batch_id=cast(BatchId, batch_id)
            )
...
async def test_background_task(
    background_tasks: BackgroundTasks, test_data: str = "test_value"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            background_tasks.add_task(
                simple_test_task, test_id=test_id, additional_data=test_data
            )
