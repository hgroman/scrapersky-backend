...
from sqlalchemy import asc, desc, func, text  # Import asc, desc
...
async def list_domains(
    ...
    sort_by: Optional[str] = Query("updated_at", description=f"Field to sort by. Allowed fields: {', '.join(ALLOWED_SORT_FIELDS.keys())}"),
    ...
    domain_filter: Optional[str] = Query(None, description="Filter by domain name (case-insensitive, partial match)")
):
    """
    ...
    logger.info(f"User {user_sub} listing domains: page={page}, size={size}, sort_by={sort_by}, sort_desc={sort_desc}, curation_status={sitemap_curation_status}, domain_filter={domain_filter}")

    # Validate sort_by field
    if sort_by and sort_by not in ALLOWED_SORT_FIELDS:
        raise HTTPException(status_code=400, detail=f"Invalid sort field '{sort_by}'. Allowed fields: {', '.join(ALLOWED_SORT_FIELDS.keys())}")
    ...
    sort_column = ALLOWED_SORT_FIELDS.get(sort_column_key, Domain.updated_at) # Use default sort
    ...
    if sitemap_curation_status:
        ...
        logger.debug(f"Applying filter: sitemap_curation_status == {sitemap_curation_status}")
    if domain_filter:
    ...
    count_query = select(func.count(Domain.id)).select_from(Domain).where(*filters) # Reuse filters
    count_result = await session.execute(count_query)
    total = count_result.scalar_one()
    logger.debug(f"Total domains matching filters: {total}")

    # --- Apply Sorting and Pagination ---
    query = base_query.order_by(sort_direction(sort_column)).offset((page - 1) * size).limit(size)
    ...
    return PaginatedDomainResponse(
        items=[DomainRecord.model_validate(item) for item in items], # Use model_validate for Pydantic v2
        total=total,
        page=page,
        size=size,
        pages=pages
...
async def update_domain_sitemap_curation_status_batch(
    ...
    logger.info(f"User {user_sub} requesting batch sitemap curation update for {len(request_body.domain_ids)} domains to status '{request_body.sitemap_curation_status.value}'")
    ...
    try:
        db_curation_status = SitemapCurationStatusEnum[api_status.name] # Use .name for reliable mapping
    except KeyError:
        logger.error(f"Invalid API status value received: {api_status.value}")
        raise HTTPException(status_code=400, detail=f"Invalid sitemap curation status value: {api_status.value}")
    ...
    if len(domains_to_update) != len(domain_ids):
        ...
        logger.warning(f"Could not find all requested domains. Missing IDs: {missing_ids}")
    ...
    try:
        for domain in domains_to_update:
            ...
            updated_count += 1
            logger.debug(f"Updating domain {domain.id} sitemap_curation_status to {db_curation_status.value}")
    ...
    logger.info(f"Batch sitemap curation update completed by {user_sub}. Updated: {updated_count}, Queued for analysis: {queued_count}")
