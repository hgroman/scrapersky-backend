...
from ..models.api_models import BatchStatusResponse
...
from ..services.sitemap.processing_service import (
    SitemapScrapingRequest as ServiceRequest,
)
...
def is_feature_check_disabled() -> bool:
    ...
    return (
        ...
        os.getenv("TESTING_MODE", "").lower() == "true"  # Add testing mode environment variable
    )
...
async def check_sitemap_access(user: dict = Depends(user_dependency), session: AsyncSession = Depends(get_session_dependency)):
...
async def scan_domain(
    ...
    try:
        ...
        async with session.begin():
            ...
            logger.info(f"Added background task for domain processing: {request.base_url}, job_id: {job_id}")
