    ...
    def find_spec(self, fullname, path, target=None):
        try:
            ...
            if caller_frame:
                ...
                if (
                    caller_filename
                    and __file__ not in caller_filename
                    and "logging/__init__.py" not in caller_filename
                ):
                    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
                    print(
                        f"{timestamp} - RUNTIME_IMPORT: {fullname} - FROM: {caller_filename}:{lineno}",
                        flush=True,
                    )
            # else:
            ...
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
            print(
                f"{timestamp} - ERROR: Exception in import hook for {fullname}: {e}",
                flush=True,
            )
