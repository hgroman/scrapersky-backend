...
SUPABASE_KEY = os.getenv("SUPABASE_SERVICE_ROLE_KEY") # Using service role for server-side script
...
async def get_openai_embedding(text: str, client: AsyncOpenAI) -> list[float] | None:
    ...
    try:
        ...
        logger.info(f"Embedding received successfully with {len(embedding)} dimensions.")
        return embedding
...
async def perform_db_search(
    ...
    try:
        ...
        logger.info(f"Executing RPC call to '{DB_FUNCTION_NAME}' with filter: {metadata_filter}")
...
async def main():
    parser = argparse.ArgumentParser(description='Perform semantic search on the database.')
    parser.add_argument('query_text', type=str, help='The text to search for.')
    parser.add_argument('--mode', type=str, choices=['titles', 'full'], default='full', help='Output mode for text format: titles or full content.')
    ...
    parser.add_argument('--filter', type=str, default='{}', help='JSON string for metadata filtering (e.g., \'{"key": "value"}\').')
    ...
    if search_results is not None:
        ...
        if args.format == 'json':
            ...
            if not search_results:
                ...
                for item in search_results:
                    print(f"- ID: {item['id']}, Similarity: {item['similarity']:.4f}, Title: {item['title']}")
    else:
