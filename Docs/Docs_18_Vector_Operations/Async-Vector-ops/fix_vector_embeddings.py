        ...
        try:
            ...
            try:
                search_results = await conn.fetch(
                    "SELECT * FROM search_docs($1, 0.5) LIMIT 3;", test_query
                )
                ...
                logger.info(
                    "Created basic search_docs function with random vector for testing"
                )
                ...
                for i, result in enumerate(search_results):
                    logger.info(
                        f"Search result {i + 1} with new function: ID={result['id']}, Title={result['title']}, Similarity={result['similarity']}"
                    )
