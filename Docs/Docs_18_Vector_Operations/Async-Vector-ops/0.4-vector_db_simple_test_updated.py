    ...
    try:
        ...
        try:
            db_results = await conn.fetch(
                "SELECT * FROM search_docs($1, $2) LIMIT 5;", test_pattern, 0.5
            )

            if db_results:
                logger.info(
                    f"Found {len(db_results)} matching documents using search_docs function:"
                )
            ...
            logger.info(
                "You may need to update the search_docs function in the database."
            )
