    ...
    try:
        ...
        try:
            ...
            if count > 0:
                ...
                logger.info(
                    f"Sample document: ID={sample['id']}, Title={sample['title']}"
                )
                ...
                if embedding_str:
                    ...
                    embedding_preview = (
                        f"[{embedding_str[:50]}...{embedding_str[-50:]}]"
                    )
        ...
        try:
            same_vectors = await conn.fetchval(
                "SELECT '[1,1,1]'::vector <=> '[1,1,1]'::vector;"
            )
            logger.info(f"Cosine distance between identical vectors: {same_vectors}")

            diff_vectors = await conn.fetchval(
        ...
        try:
            search_results = await conn.fetch(
                "SELECT * FROM search_docs('test pattern', 0.5) LIMIT 3;"
            )
            if search_results:
                for i, result in enumerate(search_results):
                    logger.info(
                        f"Search result {i + 1}: Title={result['title']}, Similarity={result['similarity']}"
                    )

                    # Check for NaN in similarity
                    if result["similarity"] != result["similarity"]:  # NaN check
                        logger.error(
                            f"NaN detected in similarity for document: {result['title']}"
                        )
            else:
        ...
        try:
            ...
            if nan_check:
                logger.error(
                    f"Found {len(nan_check)} documents with potential NaN values in embeddings:"
                )
