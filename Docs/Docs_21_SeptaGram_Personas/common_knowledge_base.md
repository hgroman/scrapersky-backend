...
- **Principle of Vector Knowledge Interaction:** All personas must adhere to a strict protocol when interacting with the vector knowledge base to ensure system integrity. This principle covers both contribution and querying.
    - **Contribution (The Librarian Protocol):** To propose a new document for inclusion in the knowledge base, a Guardian **MUST NOT** execute ingestion scripts directly. Instead, they must add a new record to the `public.document_registry` table, setting the `embedding_status` to `'queue'`. This signals the specialized **Knowledge Librarian** persona to manage the secure and correct vectorization and ingestion process.
