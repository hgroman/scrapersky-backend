...
class RegistryUpdateManager:
    ...
    async def mark_document_for_update(self, document_path):
        ...
        if result:
            logger.info(
                f"Document '{result['title']}' (ID: {result['id']}) marked for update."
            )
            return True
        else:
    ...
    async def mark_directory_for_update(self, directory_path):
        ...
        if count > 10:
            logger.warning(
                f"You've marked {count} documents for update. This may impact system performance."
            )

        logger.info(
            f"Marked {count} document(s) in directory '{directory_path}' for update."
        )
        return count
    ...
    async def mark_pattern_for_update(self, pattern):
        ...
        logger.info(
            f"Marked {count} document(s) matching pattern '{pattern}' for update."
        )
        return count
    ...
    async def list_documents_for_update(self):
        ...
        for record in records:
            last_checked = (
                record["last_checked"].strftime("%Y-%m-%d %H:%M")
                if record["last_checked"]
                else "Never"
            )
            print(
                f"{record['id']:<5} | {record['title']:<30} | {record['embedding_status']:<15} | {last_checked:<20}"
            )
    ...
    async def clear_update_flag(self, document_path):
        ...
        if result:
            logger.info(
                f"Update flag cleared for document '{result['title']}' (ID: {result['id']})."
            )
            return True
        else:
            logger.error(
                f"Document not found or not marked for update: {document_path}"
            )
            return False
...
async def main():
    ...
    parser.add_argument(
        "--mark-for-update", help="Mark a specific document for update by path or title"
    )
    parser.add_argument(
        "--mark-directory-for-update",
        help="Mark all documents in a directory for update",
    )
    parser.add_argument(
        "--mark-pattern-for-update", help="Mark documents matching a pattern for update"
    )
    parser.add_argument(
        "--list-updates",
        action="store_true",
        help="List documents currently marked for update",
    )
    parser.add_argument(
        "--clear-update", help="Clear update flag for a specific document"
    )
    parser.add_argument(
        "--clear-all-updates",
        action="store_true",
        help="Clear update flags for all documents",
    )
    ...
    try:
        ...
        if args.mark_directory_for_update:
            count = await manager.mark_directory_for_update(
                args.mark_directory_for_update
            )
            if count > 10:
                confirm = input(
                    f"You're about to mark {count} documents for update. Continue? (y/n): "
                )
                if confirm.lower() != "y":
        ...
        if args.clear_all_updates:
            ...
            if count > 0:
                logger.info(
                    f"Successfully cleared update flags for {count} document(s)."
                )
