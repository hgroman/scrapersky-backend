...
class RegistryArchiveManager:
    ...
    async def find_missing_files(self):
        ...
        for record in records:
            ...
            if not relative_db_path:  # Skip if path is empty or None
                logger.warning(
                    f"Record ID {record['id']} ('{record['title']}') has empty file_path, skipping."
                )
                continue
            ...
            if not absolute_file_path.exists():
                ...
                logger.warning(
                    f"Path exists but is not a file for '{record['title']}': {absolute_file_path}"
                )
    ...
    async def list_missing_files(self):
        ...
        for record in missing_files:
            print(
                f"{record['id']:<5} | {record['title']:<30} | {record['embedding_status']:<15} | {record['file_path']:<50}"
            )
    ...
    async def interactive_scan(self):
        ...
        for i, record in enumerate(missing_files, 1):
            print(
                f"{i:<3} | {record['id']:<5} | {record['title']:<30} | {record['embedding_status']:<15}"
            )
        ...
        while True:
            ...
            if choice == "1":
                confirm = input(
                    f"Are you sure you want to mark ALL {len(missing_files)} files as archived? (y/n): "
                )
                if confirm.lower() == "y":
                    for record in missing_files:
                ...
                selections = input(
                    "Enter file numbers to mark as archived (comma-separated, e.g., 1,3,5): "
                )
                try:
                    selected_indices = [int(x.strip()) for x in selections.split(",")]
                    for idx in selected_indices:
                        if 1 <= idx <= len(missing_files):
                            await self.mark_file_as_archived(
                                missing_files[idx - 1]["id"]
                            )
                            logger.info(
                                f"Marked file {missing_files[idx - 1]['title']} as archived."
                            )
                        else:
    ...
    async def mark_file_as_archived(self, file_id):
        ...
        if result:
            logger.info(
                f"File '{result['title']}' (ID: {result['id']}) marked as archived."
            )
            return True
        else:
    ...
    async def list_archived_files(self):
        ...
        for record in records:
            print(
                f"{record['id']:<5} | {record['title']:<30} | {record['file_path']:<50}"
            )
    ...
    async def list_all_files(self):
        ...
        for record in records:
            print(
                f"{record['id']:<5} | {record['title']:<50} | {record['embedding_status']:<15} | {record['file_path']}"
            )
        ...
        for record in records:
            ...
            if current_path.is_absolute():
                try:
                    ...
                    logger.info(
                        f"Normalized path for ID {record['id']} ('{record['title']}'): '{current_path}' -> '{relative_path}'"
                    )
                    normalized_count += 1
                except ValueError:
                    logger.warning(
                        f"Could not make path relative for ID {record['id']} ('{record['title']}'): '{current_path}' is not under project root '{project_root}'. Skipping."
                    )
...
async def main():
    ...
    parser.add_argument(
        "--scan",
        action="store_true",
        help="Scan for missing files and interactively mark them as archived.",
    )
    parser.add_argument(
        "--list-missing",
        action="store_true",
        help="List files that no longer exist at their specified paths.",
    )
    parser.add_argument(
        "--mark-archived",
        type=int,
        metavar="FILE_ID",
        help="Mark a specific file as archived by its ID.",
    )
    parser.add_argument(
        "--mark-archived-by-path",
        type=str,
        metavar="FILE_PATH",
        help="Mark a specific file as archived by its path.",
    )
    parser.add_argument(
        "--list-archived",
        action="store_true",
        help="List all files currently marked as archived.",
    )
    parser.add_argument(
        "--list-all", action="store_true", help="List all documents in the registry."
    )
    parser.add_argument(
        "--normalize-paths",
        action="store_true",
        help="Convert all absolute file_paths in the registry to relative paths.",
    )
