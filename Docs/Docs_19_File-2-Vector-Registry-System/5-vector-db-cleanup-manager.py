...
class VectorDBCleanupManager:
    ...
    async def list_cleanup_candidates(self):
        ...
        if not candidates:
            logger.info("No vector DB entries found corresponding to 'archived' documents in the registry.")
            return []
        ...
        for record in candidates:
            print(f"{record['id']:<15} | {record['title']:<70} | {record['registry_file_path']:<70}")
    ...
    async def remove_vector_entry(self, entry_id):
        ...
        if result:
            logger.info(f"Entry '{result['title']}' (Vector DB ID: {result['id']}) removed from the vector database.")
            return True
        else:
            logger.error(f"Entry with Vector DB ID {entry_id} not found in the vector database.")
            return False
    ...
    async def remove_archived_from_vector_db(self, auto_approve=False):
        ...
        if not candidates:
            logger.info("No vector DB entries to remove based on 'archived' status in registry.")
            return 0
        ...
        if not auto_approve:
            confirm = input(f"Are you sure you want to remove these {len(candidates)} entries? (y/n): ")
            if confirm.lower() != 'y':
        ...
        logger.info(f"Successfully removed {removed_count} out of {len(candidates)} targeted entries from the vector database.")
        return removed_count

async def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Vector Database Cleanup Manager - Removes entries from vector DB if marked 'archived' in registry.")
    parser.add_argument(
        "action",
