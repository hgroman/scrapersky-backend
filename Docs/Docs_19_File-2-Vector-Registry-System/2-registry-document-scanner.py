...
class DocumentRegistryManager:
    ...
    async def scan_filesystem(self):
        ...
        if not approved_dirs:
            logger.warning(
                "No approved directories found. Use 1-registry-directory-manager.py to approve directories."
            )
            return

        logger.info(
            f"Starting scan of {len(approved_dirs)} approved director(y/ies)..."
        )
        for dir_path in approved_dirs:
            dir_obj = Path(dir_path)
            if not dir_obj.is_dir():  # Check if it's actually a directory
                logger.warning(
                    f"Approved path is not a directory or not found: {dir_path}"
                )
                continue
    ...
    async def get_approved_directories(self):
        ...
        try:
            ...
            if not exists:
                logger.warning(
                    "Approved directories table doesn't exist. Run directory_approval.py --setup"
                )
                return []
        ...
        await self._update_registry(
            {
                ...
                "last_seen_at": datetime.now(
                    timezone.utc
                ),  # Renamed from last_checked and made timezone aware
        ...
        if existing_record:
            if is_v_prefixed:
                if existing_record["old_file_hash"] != doc.get("file_hash"):
                    ...
                    logger.info(
                        f"Content changed for {doc['title']}. Marked for re-vectorization."
                    )
                else:
                    current_embedding_status = existing_record[
                        "old_embedding_status"
                    ]  # Preserve existing status
                    current_needs_update = existing_record[
                        "old_needs_update"
                    ]  # Preserve existing flag
                    logger.info(
                        f"Content unchanged for {doc['title']}. Status preserved."
                    )
            ...
            await self.conn.execute(
                ...
                doc["key_concepts"],
                doc["primary_purpose"],
                current_embedding_status,
                doc["last_seen_at"],
                doc.get("file_hash"),
                current_needs_update,
                existing_record["id"],
            )
            ...
            if is_v_prefixed:
                current_embedding_status = "queue"
                current_needs_update = (
                    False  # New files are queued but don't 'need update' yet
                )
    ...
    def _determine_document_type(self, filename, path):
        ...
        for doc_type, patterns in doc_type_patterns.items():
            for pattern in patterns:
                if (
                    pattern.lower() in lowercase_name
                    or pattern.lower() in lowercase_path
                ):
                    return doc_type
    ...
    def _extract_key_concepts(self, content, max_concepts=5):
        ...
        capitalized_phrases = re.findall(
            r"\b[A-Z][a-zA-Z0-9]*([ -][A-Z][a-zA-Z0-9]*)+\b", content
        )
    ...
    async def mark_file_for_vectorization(self, file_path_str):
        ...
        if not self.conn:
            ...
            print(
                "Error: Database connection is required for marking. Please ensure the script is run in a mode that establishes a DB connection for this operation."
            )
            return
        ...
        if not original_filename.startswith("v_"):
            ...
            try:
                ...
                logger.error(
                    f"Error renaming file {file_path_str} to {target_filename}: {e}"
                )
                return
        else:
            logger.info(
                f"File {original_filename} is already prefixed with 'v_'. Ensuring registry entry is minimal and up-to-date."
            )
        ...
        logger.info(
            f"Ensured minimal registry entry for {target_filename} with embedding_status='queue'."
        )
...
async def main():
    ...
    parser.add_argument(
        "--scan",
        action="store_true",
        help="Scan filesystem for documents (uses approved directories)",
    )
    parser.add_argument(
        "--mark",
        help="Mark a document for vectorization by adding v_ prefix and making a minimal registry entry",
    )
    ...
    try:
        ...
        if args.scan:
            logger.info(
                "Starting filesystem scan for v_ documents in approved directories..."
            )
