...
async def main():
    ...
    parser.add_argument(
        "--list-approved", action="store_true", help="List approved directories"
    )
    parser.add_argument(
        "--list-candidates", help="List vectorization candidates in directory"
    )
    # NEW: Added status command
    parser.add_argument(
        "--status",
        action="store_true",
        help="Show approved directories with file counts",
    )
