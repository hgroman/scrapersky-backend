...
2.  **File:** `static/js/sitemap-curation-tab.js`
    ...
    - **Function:** (Likely) `sitemapBatchUpdate()` or similar function attached to the update button.
      ...
      - Sends a `PUT` request to `/api/v3/sitemap-files/status` with IDs and `status` in the request body (`SitemapFileBatchUpdate` schema).
...
1.  **File:** `src/routers/sitemap_files.py`
    ...
    - **Function:** `update_sitemap_files_status_batch(...)`, handling `PUT /api/v3/sitemap-files/status`.
      ...
      - **Delegates** the core update and queueing logic to the service layer.
...
3.  **File:** `src/services/page_scraper/processing_service.py` (or similar)
    - **Role:** **(Intended Target - Not Called)** This service likely contains the logic required to process a `SitemapFile` marked as `Queued`. It would fetch the sitemap, parse URLs, and potentially queue individual page scraping jobs. This service **is not currently being triggered** by the scheduler for this workflow.
...
2.  **File:** `src/schemas/sitemap_file.py`
    - **Role:** Defines Pydantic schemas for API interaction.
...
1.  **File:** `tests/routers/test_sitemap_files.py` (Likely location)
    - **Potential Test Functions:**
      - `test_update_sitemap_files_status_batch_selected_queues`: Verify calling the endpoint with status 'Selected' results in `deep_scrape_process_status` being set to `Queued` in the DB (via service call).
2.  **File:** `tests/services/test_sitemap_files_service.py` (Likely location)
    - **Potential Test Functions:**
      - `test_update_curation_status_batch_selected`: Verify service function updates curation status to 'Selected' AND process status to 'Queued'.
      - `test_update_curation_status_batch_selected_already_processing`: Verify service function updates curation status but does _not_ update process status if it's already 'Processing'.
      - `test_update_curation_status_batch_other_status`: Verify service function only updates curation status and does _not_ touch process status for non-'Selected' inputs.
3.  **File:** `tests/services/test_sitemap_scheduler.py` (Likely location)
    - **Potential Test Functions:**
      - **(Missing Test):** There should ideally be a test like `test_process_pending_jobs_picks_up_queued_sitemap_files` which verifies the scheduler queries `sitemap_files` and calls the appropriate processing service. This test would currently fail or be absent.
...
3.  Backend Router (`sitemap_files.py`) receives the call and delegates to the service.
4.  Backend Service (`sitemap_files_service.update_curation_status_batch`) implements **Dual-Status Update Logic**: updates `sitemap_files.deep_scrape_curation_status` to `Selected` and sets `sitemap_files.deep_scrape_process_status` to `Queued` (if not already processing).
...
- **Dual-Status Update Logic:** Correctly implemented in the service layer (`sitemap_files_service.py`). Setting curation status to `Selected` correctly queues the item by setting the process status.
- **No Unused Parameters:** No unused trigger parameters were identified in the relevant router/service functions.
...
- The "Dual-Status Update Logic" is again present in the service layer and could potentially be generalized.
