...
**2.3 [API Layer 3: Routers: Request Validation & Layer 1: ENUM Handling]**
...
**2.10 [Database Layer 1: Models: Place]**
...
**2.11 [Layer 1: ENUM Handling: PlaceStatusEnum, DeepScanStatusEnum]**
...
**2.12 [Layer 5: Configuration: Scheduler Settings]**
...
| ---- | -------------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------ | ------------------------------------------------------------------------------------- | ---------------------------------------- |
| 2.1  | Layer 6: UI Components Interaction                 | static/js/staging-editor-tab.js                                                | User selects rows, sets status, triggers update        | API Std ([Guide](../Docs_1_AI_GUIDES/15-API_STANDARDIZATION_GUIDE.md)), Feedback      | Verify JS details                        |
| 2.2  | API Layer 3: Routers: JWT Auth                     | src/routers/places_staging.py [NOVEL]                                          | JWT auth at router only                                | Auth Boundary ([Guide](../Docs_1_AI_GUIDES/11-AUTHENTICATION_BOUNDARY.md))            | Uses src/auth/jwt_auth.py                |
| 2.3  | API Layer 3: Routers: Validation/Layer 1: ENUM     | src/routers/places_staging.py [NOVEL], src/models/api_models.py [SHARED]       | Validates input, checks ENUMs                          | API Std, Enum Handling, Conn Mgmt, Txn Boundary, Code Org, UUID Std                   | ENUMs in api_models.py                   |
| 2.4  | API Layer 3: Routers: Layer 4: Services Delegation | src/routers/places_staging.py [NOVEL]                                          | Delegates to service or inline logic                   | Delegation, Code Org                                                                  | Service preferred, else inline           |
| 2.5  | Layer 4: Services: Status Update Logic             | src/services/places_staging_service.py [SHARED]                                | Validates, cleans, orchestrates status update          | Validation, Enum Handling, Error Handling, Code Org, ORM Req                          | Centralizes logic, uses ORM              |
| 2.6  | Layer 4: Services: Dual-Status Update              | src/services/places_staging_service.py [SHARED], src/models/place.py [SHARED]  | Updates place.status, deep_scan_status with ORM        | ORM Req ([Guide](../Docs_1_AI_GUIDES/01-ABSOLUTE_ORM_REQUIREMENT.md)), Error Handling | Batch ops, error logging                 |
...
| 2.10 | DB Layer 1: Models: Place                          | src/models/place.py [SHARED]                                                   | Defines ORM model for Place                            | ORM Req ([Guide](../Docs_1_AI_GUIDES/01-ABSOLUTE_ORM_REQUIREMENT.md)), Code Org       | Models match DB, enums in place.py       |
