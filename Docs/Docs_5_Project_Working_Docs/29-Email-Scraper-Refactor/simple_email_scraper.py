...
def crawl_website(
    start_url: str, max_pages: int = 10, verbose: bool = False
) -> List[Dict[str, Any]]:
    ...
    print(
        f"Starting crawl of {start_url} (domain: {domain}) with max_pages={max_pages}"
    )
    ...
    while to_visit and pages_scanned < max_pages:
        ...
        if verbose:
            print(
                f"Found {len(emails)} emails, {len(new_links)} new links. "
                + f"Total emails so far: {len(all_emails)}. Queue size: {len(to_visit)}"
            )

    print(
        f"Crawl complete. Scanned {pages_scanned} pages, found {len(all_emails)} emails."
    )
    return all_emails
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run test on sample HTML data"
    )
    ...
    try:
        ...
        if emails:
            ...
            for email in emails[:5]:  # Show up to 5 emails
                print(
                    f"- {email['email']} ({email['type']}) from {email['source_url']}"
                )
