...
def run_command(cmd):
    ...
    print(f"Running: {cmd}")
    process = subprocess.run(cmd, shell=True, cwd=ROOT_DIR,
...
def main():
    ...
    for file in ["codebase_analysis_report.html", "enhanced_analysis_report.html",
                "enhanced_analysis_report.json", "dependency_graph.json"]:
        if os.path.exists(os.path.join(ROOT_DIR, file)):
            shutil.copy2(os.path.join(ROOT_DIR, file),
