    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Error creating batch: {response.status_code} - {response.text}")
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Error getting status: {response.status_code} - {response.text}")
                return {"error": response.text, "status_code": response.status_code}
        except Exception as e:
...
async def monitor_batch_completion(batch_id: str, timeout: int = 120) -> Dict[str, Any]:
    ...
    while time.time() - start_time < timeout:
        ...
        if current_status != last_status or abs(current_progress - last_progress) > 0.05:
            last_status = current_status
            last_progress = current_progress
            logger.info(f"Batch status: {current_status}, progress: {current_progress:.2f}")
...
async def run_test(test_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if actual_status in status_mapping[expected_status]:
        if expected_status == "partial":
            ...
            if final_status.get("completed_domains", 0) > 0 and final_status.get("failed_domains", 0) > 0:
                logger.info(f"Test passed: {test_name}")
                results["status"] = "pass"
            else:
                logger.warning(f"Expected mixed results but got all success or all failure")
                results["status"] = "fail"
        else:
            logger.info(f"Test passed: {test_name}")
            results["status"] = "pass"
    else:
        logger.warning(f"Test failed: Expected status {expected_status} but got {actual_status}")
        results["status"] = "fail"
