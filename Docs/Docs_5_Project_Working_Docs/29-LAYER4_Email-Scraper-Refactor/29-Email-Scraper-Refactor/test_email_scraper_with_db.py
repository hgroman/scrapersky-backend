...
def extract_emails_from_soup(soup: BeautifulSoup) -> List[Tuple[str, str]]:
    ...
    for field in soup.find_all(["input", "textarea"]):
        if isinstance(field, Tag):
            ...
            if (
                (
                    any(name in field_name for name in email_field_names)
                    or any(name in field_id for name in email_field_names)
                )
    ...
    for meta in soup.find_all("meta"):
        if isinstance(meta, Tag):
            ...
            if isinstance(content, str) and "@" in content and "." in content:
                ...
                for pattern in [
                    EMAIL_PATTERNS["general"],
                    EMAIL_PATTERNS["obfuscated"],
                ]:
...
async def scrape_url_and_store(
    url: str, domain_obj: Domain, job_obj: Job, visited: Set[str], session
) -> Tuple[int, Set[str]]:
        ...
        if all_emails:
            ...
            for email, context in all_emails:
                ...
                contact_query = (
                    select(Contact.id)
                    .where(
                        Contact.domain_id == domain_obj.id, Contact.email == email_lower
                    )
                    .limit(1)
                )
                ...
                if not contact_exists:
                    ...
                    logger.info(
                        f"Found new contact: {email_lower} ({email_type}) on page {url}"
                    )
...
async def test_email_scraper(
    domain_name: str, max_pages: int = 10, verbose: bool = False
):
    ...
    async with get_background_session() as session:
        try:
            ...
            if hasattr(Domain, "get_by_domain_name"):
                ...
                domain_query = (
                    select(Domain).where(Domain.domain == domain_name).limit(1)
                )
            ...
            if domain_obj:
                logger.info(
                    f"Found existing domain: {domain_name} (ID: {domain_obj.id})"
                )
            else:
                # Create a new domain
                logger.info(
                    f"Domain {domain_name} not found. Creating new domain record."
                )
                domain_obj = Domain(
                    domain=domain_name,
                    status="active",
            ...
            while to_visit and pages_scanned < max_pages:
                ...
                if verbose:
                    logger.info(
                        f"Processed page {pages_scanned}/{max_pages}: {current_url}"
                    )
                    logger.info(
                        f"Found {contacts_count} new contacts, {len(new_links)} new links"
                    )
            ...
            contact_query = select(Contact.email).where(Contact.source_job_id == job_id)
            contact_result = await session.execute(contact_query)
            contacts = contact_result.scalars().all()

            # Store results in the job
            job_obj.result_data = {"emails": list(contacts)}
            await session.commit()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test Email Scraper with Database Integration"
    )
