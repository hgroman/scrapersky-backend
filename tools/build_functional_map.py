...
if deps_google_maps_api_path.exists():
    ...
    print(f"Warning: {deps_google_maps_api_path} not found. Mapping for LocalMiner will be incomplete.")
...
if deps_places_staging_path.exists():
    ...
    print(f"Warning: {deps_places_staging_path} not found. Mapping for StagingEditor will be incomplete.")
    deps_places_staging = []
...
if deps_domains_path.exists():
    ...
    print(f"Warning: {deps_domains_path} not found. Mapping for DomainCuration will be incomplete.")
    deps_domains = []
...
if deps_sitemap_files_path.exists():
    ...
    print(f"Warning: {deps_sitemap_files_path} not found. Mapping for SitemapCuration will be incomplete.")
    deps_sitemap_files = []
...
if deps_modernized_page_scraper_path.exists():
    ...
    print(f"Warning: {deps_modernized_page_scraper_path} not found. Mapping for DomainScanning will be incomplete.")
...
if deps_modernized_sitemap_path.exists():
    ...
    print(f"Warning: {deps_modernized_sitemap_path} not found. Mapping for ContentMap will be incomplete.")
...
if deps_batch_sitemap_path.exists():
    ...
    print(f"Warning: {deps_batch_sitemap_path} not found. Mapping for BatchProcessing (Sitemap) will be incomplete.")
    deps_batch_sitemap = []
...
if deps_db_portal_path.exists():
    ...
    print(f"Warning: {deps_db_portal_path} not found. Mapping for DBPortal will be incomplete.")
    deps_db_portal = []
...
if deps_dev_tools_path.exists():
    ...
    print(f"Warning: {deps_dev_tools_path} not found. Mapping for DevTools will be incomplete.")
    deps_dev_tools = []
...
feature_deps = {
    ...
    "LocalBusinessCuration": ("src/routers/local_businesses.py", deps_local_business_curation),
    "DomainCuration": ("src/routers/domains.py", deps_domains),
    "SitemapCuration": ("src/routers/sitemap_files.py", deps_sitemap_files),
    "DomainScanning": ("src/routers/modernized_page_scraper.py", deps_modernized_page_scraper),
...
for business_function, (entry_point, dep_list) in feature_deps.items():
    for filepath in dep_list:
        if filepath in functional_map:
            ...
            if business_function not in functional_map[filepath]["Mapped Business Function(s)"]:
                functional_map[filepath]["Mapped Business Function(s)"].append(business_function)
...
for filepath, data in functional_map.items():
    ...
    if num_mappings > 1:
        ...
        if feature == "BatchProcessing": batch_files.add(filepath)
        if feature == "DBPortal": db_portal_files.add(filepath)
        if feature == "DevTool": dev_tool_files.add(filepath)
...
report_path = Path("Docs/Docs_0_Architecture_and_Status") / "functional_dependency_map.json" # NEW PATH
report_path.parent.mkdir(parents=True, exist_ok=True)

output_data = {
    "summary": summary,
    "file_mappings": functional_map
}
