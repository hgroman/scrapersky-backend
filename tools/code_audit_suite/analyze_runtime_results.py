    ...
    for line in logs:
        if LOG_SEARCH_PREFIX in line:
            try:
                ...
                print(f"Warning: Could not parse module from log line: '{line}'. Error: {e}")
...
def path_to_module(file_path: str, src_root_abs: str) -> str | None:
    ...
    if not file_path.startswith(src_root_abs_norm):
         ...
         if file_path == os.path.normpath(src_root_abs) and os.path.basename(file_path) == '__init__.py':
             # Special case for src_root/__init__.py
             pass # Will be handled below
         else:
            print(f"Warning: File '{file_path}' not under src_root '{src_root_abs_norm}'. Skipping.")
            return None
    ...
    if not module_path.startswith(src_dir_name + '.') and module_path != src_dir_name:
         if module_path: # Avoid prefixing if module_path became empty (e.g., was just __init__)
            module_path = f"{src_dir_name}.{module_path}"
         elif os.path.basename(file_path) == '__init__.py': # Handle /app/src/__init__.py explicitly
    ...
    for root, dirs, files in os.walk(src_dir_abs, topdown=True):
        ...
        for file in files:
            if file.endswith('.py'):
                ...
                current_src_root = os.path.dirname(src_dir_abs) if file == '__init__.py' and root == src_dir_abs else src_dir_abs
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze runtime results to find unused modules.")
    parser.add_argument('--src-dir', default='/app/src', help='Directory containing source code.')
    parser.add_argument('--report-dir', default='/app/reports', help='Directory to save the report.')
    ...
    print(f"--- Reading logs from stdin --- ")
    ...
    print(f"Found {len(loaded_modules)} unique loaded modules matching '{LOG_SEARCH_PREFIX}*'.")
    ...
    try:
        ...
        print(f"Unused module list saved successfully.")
    except IOError as e:
