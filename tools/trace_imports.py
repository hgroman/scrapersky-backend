...
try:
    ...
    from importlab.resolve import AbsoluteResolver, RelativeResolver
...
def load_config(config_path):
    ...
    try:
        ...
        print(
            f"Warning: Config file not found at {config_path}. No exclusions applied."
        )
        return []
    ...
    try:
        ...
        for variable_name in router_variables_used:
            if variable_name in variable_to_module_map:
                ...
                relative_file_path = (
                    module_path_str.strip(".").replace(".", "/") + ".py"
                )
                router_file_path = entry_point_path.parent / relative_file_path
                router_paths.add(str(router_file_path.resolve()))
            else:
                print(
                    f"Warning: Could not map router variable '{variable_name}' to an import statement via AST."
                )
        ...
        print(
            f"Error parsing entry point {entry_point_path} for routers using AST: {e}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Trace Python module imports using importlab to find unused files."
    )
    parser.add_argument(
        "--entry",
        default=DEFAULT_ENTRY_POINT,
        help=f"Primary entry point script (default: {DEFAULT_ENTRY_POINT})",
    )
    parser.add_argument(
        "--config",
        default=DEFAULT_CONFIG_FILE,
        help=f"Path to YAML config file with exclusions (default: {DEFAULT_CONFIG_FILE})",
    )
    parser.add_argument(
        "--out-used",
        default=DEFAULT_USED_OUTPUT,
        help=f"Output JSON file for used modules (default: {DEFAULT_USED_OUTPUT})",
    )
    parser.add_argument(
        "--out-unused",
        default=DEFAULT_UNUSED_OUTPUT,
        help=f"Output JSON file for unused candidates (default: {DEFAULT_UNUSED_OUTPUT})",
    )
    args = parser.parse_args()

    print("Starting import trace using importlab...")
    ...
    project_root_path = (
        src_root_path.parent
    )  # Assumes src is directly under project root
    ...
    try:
        ...
        for router_path_str in routers_found:
            if Path(router_path_str).exists():
                ...
                print(
                    f"Warning: Router path {router_path_str} derived via AST does not exist. Skipping add."
                )
        ...
        for scheduler_path in schedulers_found:
            if Path(scheduler_path).exists():
                ...
                print(
                    f"Warning: Scheduler path {scheduler_path} does not exist. Skipping add."
                )
